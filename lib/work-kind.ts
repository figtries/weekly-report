...
/**
 * The four forms a row can be filled in with, and NOBODY IS ASKED WHICH ONE.
 *
 * A shape is worked out: from the method the importer or the planner set, and
 * for a row that was asked its kind, from the row's own name through
 * `shapeOf`. It was briefly offered as a question of its own — four buttons
 * reading Stages / Quantity / One-off / Typed percent, in front of the kind —
 * and that was wrong twice: it put a question about this app's data model in
 * front of a question about the person's work, and it asked for an answer the
 * row's name already gives.
 *
 ...
 * who reported the figure and when, so it made people classify a percentage by
 * where it came from before they were allowed to type it. Those note fields
 * are `SourceNote` now, which every form can show. Rows already saved as a
 * quote keep their stored `source` and their note, and render as `manual` with
 * that note already filled in.
 */
